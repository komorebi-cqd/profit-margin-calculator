...
export default function Calculator() {
  ...
  const { handleSubmit, handleChange, handleBlur, resetForm, touched, errors, values: { costOfItem, markup } } = useFormik({
    initialValues: {
      costOfItem: '',
      markup: '',
    },
    ...
    onSubmit: ({ costOfItem, markup }) => {
      ...
      const calculatedGrossMargin = (calculatedProfit / calculatedSalePrice) * 100;
      setForm({
        ...form,
      ...
      <div className="px-4 py-6 md:p-8 sm:p-10 lg:flex-auto items-center flex">
        ...
        >
          ...
          <div className='flex flex-col space-y-3 md:flex-row md:space-y-0 md:space-x-8'>
            ...
            <button type="submit" className='btn btn-secondary flex-1' onClick={onRest}>Reset</button>
          </div>
        </form>
      </div>
      ...
      <div className='-mt-2 p-2 lg:mt-0 lg:w-full lg:max-w-lg lg:flex-shrink-0'>
        <div className="rounded-2xl bg-gray-50 dark:bg-black text-center ring-1 ring-inset ring-gray-900/5 dark:ring-gray-700 lg:flex lg:flex-col lg:justify-center py-8">
          <div className="space-y-8">
            <Outcome label='Sale Price' value={salePrice} currency={currency} withDivider={true} />
            <Outcome label='Profit' value={profit} currency={currency} withDivider={true} />
